...
* @return none.
*
* @brief enable Init Channel Output.One-time effect,manual clear is not required
*
*/
__STATIC_INLINE void PWM_EnableInitChlOutput(PWM_Type *PWMx)
{
    PWMx->FUNCSEL |= PWM_FUNCSEL_INIT_Msk;
}
...
*
* @param[in] PWMx: pointer to one of three PWM base register address(PWM0 to PWM3).
* @param[in] val: output init value(value from 0x0 to 0x3F)
*
* @return none.
*
* @brief set PWM Channel output init value.
*
*/
__STATIC_INLINE void PWM_OutputInit_Config(PWM_Type *PWMx, uint8_t val)
{
    PWMx->OUTINIT = val;
}
/**
...
* @return none.
*
* @brief set PWM output channel init.
*
*/
__STATIC_INLINE void PWM_SetOutputChlInit(PWM_Type *PWMx, uint8_t pwmChannel)
{
    PWMx->OUTINIT |= (1UL << pwmChannel);
}
/**
*
* @param[in] PWMx: pointer to one of three PWM base register address(PWM0 to PWM3).
...
* @return none.
*
* @brief clr PWM output channel init.
*
*/
__STATIC_INLINE void PWM_ClrOutputChlInit(PWM_Type *PWMx, uint8_t pwmChannel)
{
    PWMx->OUTINIT &= ~(1UL << pwmChannel);
}
...
*
* @param[in] PWMx: pointer to one of three PWM base register address(PWM0 to PWM3).
* @param[in] val: output mask value(value from 0x0 to 0x3F)
*
* @return none.
*
* @brief PWM_OutputMask_Config, Set PWM Channel output mask value.
*
*/
__STATIC_INLINE void PWM_OutputMask_Config(PWM_Type *PWMx, uint8_t val)
{
    PWMx->OMCR = val;
}
/**
...
__STATIC_INLINE void PWM_InputCaptureFilterClr(PWM_Type *PWMx, uint8_t pwmChannel)
{
    ...
    PWMx->CAPFILTER &= ~(0x001FUL << pwmShift);
}
...
* @param[in] faultPolarity: Fault polarity
*
* @return none
*
* @brief Set Fault Polarity.
*
*/
__STATIC_INLINE void PWM_SetFaultPolarity(PWM_Type *PWMx, uint8_t faultChannel, uint8_t faultPolarity)
{
    if (faultPolarity)
    {
        PWMx->FLTPOL |= faultPolarity << faultChannel;
    }
    else
    {
        PWMx->FLTPOL &= ~(1UL << faultChannel);
    }
}
/**
*
...
__STATIC_INLINE void PWM_FaultDisable(PWM_Type *PWMx, uint8_t faultInputSelect)
{
    if (PWMx->FUNCSEL & PWM_FUNCSEL_WPDIS_Msk) //if not protected
    {
        PWMx->FFAFER  &= ~(1UL << faultInputSelect);
    }
    else                                  //if protected
    {
        PWM_WriteProtectionDisable(PWMx);
...
__STATIC_INLINE void PWM_FaultFilterDisable(PWM_Type *PWMx, uint8_t faultInputSelect)
{
    if (PWMx->FUNCSEL & PWM_FUNCSEL_WPDIS_Msk) //if not protected
    {
        PWMx->FFAFER &= ~(0x10UL << faultInputSelect);
    }
    else                                  //if protected
    {
        PWM_WriteProtectionDisable(PWMx);
