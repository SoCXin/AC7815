...
typedef struct
{
    uint8_t counterOverflow         : 1;    ///<1: overflow bit is set, 0: rno overflow
    uint8_t registerReady           : 1;    ///<1: register has been updated, 0: no up-to-date
    uint8_t softReset               : 1;    ///<1: reset PWDT, 0: no action
    uint8_t overflowIntEn           : 1;    ///<1: enable overflow interrupt, 0: disable overflow interrupt
    uint8_t readyIntEn              : 1;    ///<1: enable data ready interrupt, 0: disable data ready interrupt
    uint8_t pwdtIntEn               : 1;    ///<1: enable PWDT module interrupt, 0: disable PWDT module interrupt
    uint8_t pwdtEn                  : 1;    ///<1: enable PWDT module, 0: disable PWDT module
    uint8_t clockPreScaler          : 3;    ///<prescaler value is from 1 to 128
    uint8_t edgeMode                : 2;    ///<select edge triggers
    uint8_t pinSelect               : 2;    ///<input pin select
    uint8_t clockSource             : 1;    ///<1: PWDT clock source is alternative clock, 0: PWDT clock source is bus clock
    uint8_t pwdtTimerEn             : 1;    ///<1:Enable PWDT Timer Function; 0:Disable Timer function
    uint16_t pwdtTimerValue         : 16;   ///<Set Timer Count Value
    uint8_t compEn                  : 1;    ///<1:Enable PWDT Comp Function; 0:Disable Comp function
    uint8_t pwdtHallEn              : 1;    ///<1:Enable PWDT HallSensorDetect Function; 0:Disable Hall function
    uint8_t pwdtFilterEn            : 1;    ///<1:Enable PWDT Filter Function; 0:Disable Filter function
    uint8_t pwdtFilterPsc           : 4;    ///<Set PWDT Filter prescaler
    uint8_t pwdtFilterValue         : 4;    ///<Set PWDT Filter Value
} PWDT_ConfigType;
