...
static const uint8_t s_pwmIrqCtrlMap[PWM_NUM] = {PWM0_IRQn, PWM1_IRQn, PWM2_IRQn, PWM3_IRQn};
...
void PWM_ModuleDisable(PWM_Type *PWMx)
{
    ...
    else if (PWM2 == PWMx)
    {
        ...
        CKGEN_Enable(CLK_PWM3_TIMER, 0);  // ES1 HW issue, will be fixed in ES2
...
int32_t PWM_EnableIRQ(PWM_Type *PWMx)
{
    ...
    for (i = 0; i < PWM_NUM; i++)
    {
        ...
        {
            tmpIrq = (IRQn_Type)s_pwmIrqCtrlMap[i];
        }
    }
