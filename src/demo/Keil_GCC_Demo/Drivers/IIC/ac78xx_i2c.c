...
///< For I2C interrupt
uint32_t g_i2cMasterSendBufferLength = 16;                          ///< I2C master initial send buffer length
uint32_t g_i2cMasterReceiveBufferLength = 16;                       ///< I2C master initial receive buffer length
uint32_t g_i2cSlaveSendBufferLength = 16;                           ///< I2C slave initial send buffer length
uint32_t g_i2cSlaveReceiveBufferLength = 16;                        ///< I2C slave initial receive buffer length
...
* I2C_Initialize
*
* @param[in] I2Cx: I2C type pointer
* @param[in] config: I2C configuration structure pointer
...
*
* @param[in] I2Cx: I2C type pointer
* @param[in] callback: I2C I2C_EventCallBack
* @return 0: success, other: error value
*
* @brief  set I2C event callback function
*/
...
*
* @param[in] I2Cx: I2C type pointer
* @param[in] wrBuff: I2C write buffer pointer
* @param[in] rdBuff: I2C read buffer pointer
* @param[in] buffSize: I2C transmit buffer length
...
int32_t I2C_InitCallback(I2C_Type *I2Cx, uint8_t * wrBuff, uint8_t *rdBuff, uint32_t buffSize)
{
    ...
    g_i2cMasterSendBufferLength = buffSize;
    g_i2cMasterReceiveBufferLength = buffSize;
    g_i2cSlaveSendBufferLength = buffSize;
    g_i2cSlaveReceiveBufferLength = buffSize;

    g_i2cStatus[index] = I2C_STATUS_IDLE;
    g_i2cSlaveReceiveInfo[index].freeLength = g_i2cSlaveReceiveBufferLength;
...
*
* @brief  Get I2C slave transfer status
*/
...
* @return 0: success, other: error value
*
* @brief  Reset I2C transfer status
*/
...
* I2C_Start
*
* @param[in] I2Cx£ºI2C type pointer
* @return 0: success, other: error value
*
...
* @return 0: success, other: error value
*
* @brief  I2C read the last one byte data
*/
...
* @Param[in] wrBuff: I2C write buffer pointer
* @Param[in] dataLength: I2C data length
* @return 0: success, other: error value
*
* @brief  I2C Master send "start + address + data0~dataN + stop"
*/
int32_t I2C_MasterWrite(I2C_Type *I2Cx, uint32_t slaveAddress, uint8_t *wrBuff, uint32_t dataLength)
{
    ...
    while ((I2Cx->STATUS_1.STATUS_1_Byte & 0x28) != I2C_STATUS_1_READY_MASK);
...
* @return 0: success, other: error value
*
* @brief  I2C Master send "start + address" then read "data0~dataN" from slave, at last send "stop"
*/
...
* @return 0: success, other: error value
*
* @brief  I2C Master send "start + address10-9 + address8-1 + data0~dataN + stop"
*/
...
* @return 0: success, other: error value
*
* @brief  I2C Master read with extension 10-bit address
*/
...
*
* @param[in] index: I2C type pointer
* @param[in] wparam: reserved
* @param[in] lparam: reserved
* @return none
*
    ...
    else
    {
        ...
        else
        {
            if (g_i2cSlaveReceiveInfo[index].freeLength != 0)
            {
                ...
                if (g_i2cSlaveReceiveInfo[index].tail >= g_i2cSlaveReceiveBufferLength)
                {
...
* @param[in] I2Cx: I2C type pointer
* @param[in] wrBuff: I2C write buffer pointer
* @param[in] length: I2C data length
* @return 1: success, 0: error, send too many data in once
*
...
int32_t I2C_SlaveWrite(I2C_Type *I2Cx, uint8_t *wrBuff, uint32_t length)
{
    ...
    if (length > g_i2cSlaveSendBufferLength)
    {
        errorStatus = 0;
    }
    else
...
* @param[in] I2Cx: I2C type pointer
* @param[in] rdBuff: I2C read buffer pointer
* @param[in] length: container to save I2C slave received length
* @return 0: success, other: error value
*
...
int32_t I2C_SlaveRead(I2C_Type *I2Cx, uint8_t *rdBuff, uint32_t *length)
{
    ...
    I2C_DisableInt(I2Cx);
    while (g_i2cSlaveReceiveInfo[index].freeLength < g_i2cSlaveReceiveBufferLength)
    {
        rdBuff[tmpReadDataLength++] = g_i2cSlaveReceiveInfo[index].receiveBuff[g_i2cSlaveReceiveInfo[index].head++];
        if (g_i2cSlaveReceiveInfo[index].head >= g_i2cSlaveReceiveBufferLength)
        {
...
* @return 0: success, other: error value
*
* @brief  I2C Master send "start + address + data0~dataN + stop" by interrupt
*/
...
* @return 0: success, other: error value
*
* @brief  I2C Master read by interrupt
*/
...
* @param[in] I2Cx: I2C type pointer
* @param[in] rdBuff: I2C read buffer pointer
* @return 0: done't received all data, other: Master received data pointer
*
* @brief  check I2C Master read by interrupt
*/
